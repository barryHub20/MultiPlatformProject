...
{
	/*
		Look up table for rotation, avoids sqrt at runtime.
		Assume float percision to be at 0.01 max, so lookup table contains range from 0.0f to 1.0f
	*/
	static float mag_LookUp_XYZ[11][11][11];

public:
	/* Init lookup table */
	static void InitMagLookup();
	static float getMag(float xAxis, float yAxis, float zAxis);
	...
	//smae as *=, but better performance since no new Mtx44 will be created
	void MultiplyWith(const Mtx44& rhs);
